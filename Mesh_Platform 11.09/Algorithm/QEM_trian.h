...
namespace easy3d {
	...
	private:
		...
		bool is_collapse_legal(const CollapseData &cd, vec3 newv);

		// what is the priority of collapsing the halfedge h;塌陷半边h的优先级是什么
		float priority(const CollapseData &cd, vec3 newv);
	...
	private:
		...
		SurfaceMesh::VertexProperty<int> vin_proxy;       //#@#@#@#

		SurfaceMesh::FaceProperty<Points> face_points_;       //面属性的点数组，面的邻点
		SurfaceMesh::VertexProperty<vec3> tar_ver;
		...
		//std::vector<vec3> proxy_normals;
		//std::vector<vec3> proxy_point;
		SurfaceMesh::Vertex now_v0;
		SurfaceMesh::Vertex now_v1;

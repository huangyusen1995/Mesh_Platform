...
namespace easy3d {
	...
	void SurfaceMeshQEM::initialize(int cur_id, float aspect_ratio , float edge_length, unsigned int max_valence, float normal_deviation ) {
		...
		vin_proxy = mesh_->add_vertex_property<int>("v:in_proxy",-1);
		tar_ver = mesh_->add_vertex_property<vec3>("v:tar_ver");
		//计算代理
		//用平均值计算各个代理面的法向量
		std::vector<vec3> proxy_normals(cur_id);
		std::vector<int> proxy_facecount(cur_id);

		std::vector<vec3> proxy_point(cur_id);
		std::vector<std::vector<SurfaceMesh::Vertex>> proxy_vertices(cur_id);
		...
		if (cur_id != 0)
		{
			...
			for (auto f : mesh_->faces()) //计算每个代理面的法向量和代理面的三角面数。
			{
				if (planar_segments[f] != -1)
				{
					...
					{
						proxy_vertices[planar_segments[f]].push_back(vv);
					}
				}
			}

			for (int i = 0; i < proxy_vertices.size(); i++)
			{
				sort(proxy_vertices[i].begin(), proxy_vertices[i].end());
				proxy_vertices[i].erase(unique(proxy_vertices[i].begin(), proxy_vertices[i].end()), proxy_vertices[i].end());//删除重复的顶点
			}
			

			for (int i = 0; i < proxy_vertices.size(); i++) 
			{
				if (proxy_vertices[i].size() == 0) continue;
				for (int j = 0; j < proxy_vertices[i].size(); j++) 
				{
					proxy_point[i] += mesh_->position(proxy_vertices[i][j]);
				}
				proxy_point[i] = proxy_point[i] / proxy_vertices[i].size();
			}
			//for (int i = 0; i < proxy_point.size(); i++) std::cout << i << "代理面内点：" << proxy_point[i] << "\n";
		    ...
		    for (auto v : mesh_->vertices()) {          //对于每个顶点操作：
			    ...
			    if (!mesh_->is_isolated(v)) {               //is_isolated返回v是否被隔离，即不与任何面发生关联
				    ...
				    {
					    vquadric_[v] = Quadric(proxy_normals[planar_segments[ff]], proxy_point[planar_segments[ff]]);
						//for (auto f : mesh_->faces(v)) vquadric_[v] += Quadric(fnormal_[f], vpoint_[v]);
						vin_proxy[v] = planar_segments[ff];
				    }
				    else                                  //不都属于代理，用原始QEM
				    {
			...
			for (auto v : mesh_->vertices()) 
			{
				if (vin_proxy[v] != -1)
				{
					vec3 nn = proxy_normals[vin_proxy[v]];//法向量(a,b,c)
					vec3 M = proxy_point[vin_proxy[v]]; //(面内一点M(x0,y0,z0) )
					float t = (nn[0] * vpoint_[v][0] + nn[1] * vpoint_[v][1] + nn[2] * vpoint_[v][2] - (nn[0] * M[0] + nn[1] * M[1] + nn[2] * M[2]))/ (nn[0] * nn[0] + nn[1] * nn[1] + nn[2] * nn[2]);
					tar_ver[v][0] = vpoint_[v][0] - nn[0] * t;
					tar_ver[v][1] = vpoint_[v][1] - nn[1] * t;
					tar_ver[v][2] = vpoint_[v][2] - nn[2] * t;
				}
			}
		}
		else
	...
	void SurfaceMeshQEM::simplify(unsigned int n_vertices) {    //输入最终想要的点的数量
		...
		while (nv > n_vertices && !queue_->empty()) {
			...
			now_v0 = mesh_->source(h);
			now_v1 = mesh_->target(h);
			CollapseData cd(mesh_, h); //点v的各个数据
			/*std::cout << "***************\n"<<v << " =》cost: " << vpriority_[v];
			std::cout << " | h-out:" << mesh_->target(vtarget_[v]) << "| h-in:" << mesh_->source(vtarget_[v])<<"\n";
			
			std::cout << "=> newv: " << newv1 <<"| tar_ver"<< tar_ver[mesh_->target(vtarget_[v])] << "\n";*/
			
			//std::cout << "当前点数：" << nv << "/队列大小：" <<queue_->size() << "/v点属于代理：" << vin_proxy[v] <<"/cost："<<vpriority_[v]<<"\n";
			// check this (again)       再次检查h是否可以塌陷
			newv1 = compute_new_v(cd);
		...
		mesh_->remove_vertex_property(vin_proxy);
		mesh_->remove_face_property(normal_cone_);
	...
	void SurfaceMeshQEM::enqueue_vertex(SurfaceMesh::Vertex v) {  //对顶点v0在队列中的值进行排序整理
		...
		for (auto h : mesh_->halfedges(v)) {             //！@！@为点周围的所有向外的半边：对每个半边操作：
			...
			if (is_collapse_legal(cd, newv)) {                       //半边h是否允许塌缩?可以返回True
				prio = priority(cd, newv);                           //priority()计算这个边的二次误差度量  %%%%%%%
	...
	bool SurfaceMeshQEM::is_collapse_legal(const CollapseData &cd, vec3 newv) {
		...
		//if (vin_proxy[cd.v0] == 1 && vin_proxy[cd.v1] == 1)
		//{
		//	vec3 newv = compute_new_v(cd);
		//	vpoint_[cd.v1] = newv;
		//}
	...
	float SurfaceMeshQEM::priority(const CollapseData &cd, vec3 newv) {
		// computer quadric error metric
		Quadric Q = vquadric_[cd.v0];
		Q += vquadric_[cd.v1];        //此时Q为边v0v1的二次误差
		if (vin_proxy[cd.v1] >= 0 && vin_proxy[cd.v0] == vin_proxy[cd.v1] && vin_proxy[cd.v0] != -1)
		{
			return Q(newv);
		}
		//return abs(Q(newv));
	...
	vec3 SurfaceMeshQEM::compute_new_v(const CollapseData &cd){
		if (vin_proxy[cd.v1] >= 0 && vin_proxy[cd.v0] == vin_proxy[cd.v1] && vin_proxy[cd.v0] != -1)
		{
			//return tar_ver[cd.v1] ;
			return (tar_ver[cd.v1] + tar_ver[cd.v0])/2;
		}
	...
	void SurfaceMeshQEM::collapse_qem_LML(SurfaceMesh::Halfedge h, vec3 newv)
	{
		...
		//if (vin_proxy[mesh_->source(h)] != -1 && vin_proxy[mesh_->target(h)] != -1)
		//{
		//	SurfaceMesh::Vertex vh = mesh_->target(h);
		//	vpoint_[vh] = newv;
		//}

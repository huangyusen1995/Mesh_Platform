...
Model *ViewerQt::originModel() const {
	if (models_.empty())
		return nullptr;
	if (model_idx_ < models_.size())
	{
...
Model *ViewerQt::secondModel() const {
	if (models_.empty())
		return nullptr;
	if (model_idx_ > 0 && model_idx_ < models_.size())
	{
		return models_[1];
	}
	return nullptr;
...
Model *ViewerQt::thirdModel() const {
	if (models_.empty())
		return nullptr;
	if (model_idx_ > 1 && model_idx_ < models_.size())
	{
		return models_[2];
	}
	return nullptr;
...
Model *ViewerQt::fourthModel() const {
	if (models_.empty())
		return nullptr;
	if (model_idx_ > 2 && model_idx_ < models_.size())
	{
		return models_[3];
	}
	return nullptr;
...
Model *ViewerQt::resultModel() const {
	if (models_.empty())
		return nullptr;
	int n = models_.size();
	if (model_idx_ > 0 && model_idx_ < n)
	{
		return models_[n - 1];
	}
	return nullptr;
...
void ViewerQt::setCurrentModel(easy3d::Model *m)
{
...
void ViewerQt::setShowOnly()
{
	Model *active_model = currentModel();
	if (active_model) {
		for (auto m : models()) {

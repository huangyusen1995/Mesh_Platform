...
void Mesh_PlatformClass::runColorMap()
{
	...
	auto min_distance = result_mesh->vertex_property<float>("v:min_distance",0.0);
	std::cout << "run colormap...." << std::endl;
	float distB = 0.0;
	for (auto vb : result_mesh->vertices())
	{
		float min_distB = 100.0;
		for (auto fb : original_mesh->faces())
		{
			//original_mesh中v点减去result_mesh内一点
			vec3 bp = result_mesh->position(vb) - original_mesh->position(original_mesh->target(original_mesh->halfedge(fb)));
			//两点距离
		...
		min_distance[vb] = min_distB;
	}

	auto faces = result_mesh->renderer()->get_triangles_drawable("faces");

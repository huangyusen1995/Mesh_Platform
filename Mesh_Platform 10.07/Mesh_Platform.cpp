...
void Mesh_PlatformClass::createActionsForErrorMenu() {
	...
	connect(ui->actionColormMap, SIGNAL(triggered()), this, SLOT(runColorMap()));
...
void Mesh_PlatformClass::runColorMap()
{
	SurfaceMesh *original_mesh = dynamic_cast<SurfaceMesh *>(viewer_->originModel());
	SurfaceMesh *result_mesh = dynamic_cast<SurfaceMesh *>(viewer_->resultModel());

	//result_mesh->update_face_normals();
	//SurfaceMesh::FaceProperty<vec3> fnormal = result_mesh->get_face_property<vec3>("f:normal");
	auto min_distance = original_mesh->vertex_property<float>("v:min_distance",0.0);
	std::cout << "run colormap...." << std::endl;
	float dist = 0.0;
	for (auto v : original_mesh->vertices())
	{
		...
		{
			vec3 ap = original_mesh->position(v) - result_mesh->position(result_mesh->target(result_mesh->halfedge(f)));  //original_mesh中v点减去result_mesh内一点
			//vec3 nn = fnormal[f]; 
			vec3 nn = result_mesh->compute_face_normal(f);
			//面法向量方法待定
			dist = (abs(dot(nn, ap)) / sqrt(nn[0] * nn[0] + nn[1] * nn[1] + nn[2] * nn[2]));                        //点面距
	...
	//const std::string texture_file = "E:\workspace\Mesh_Platform\resources\colormaps\rainbow.png";
	...
	//original_mesh->renderer()->set_visible(original_mesh);
	viewer_->update();
	updateUi();
